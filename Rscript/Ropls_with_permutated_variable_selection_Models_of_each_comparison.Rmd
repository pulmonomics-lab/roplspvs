...
if (nrow(subsetdatamatrix_group1<7)) {set_crossvalI <- nrow(subsetdatamatrix_group1)} else {set_crossvalI <- 7}
...
if (nrow(subsetdatamatrix_group2<7)) {set_crossvalI <- nrow(subsetdatamatrix_group2)} else {set_crossvalI <- 7}
...
```{R permutation pre vs of model1}
...
```{R  plot permutation pre vs Model1, fig.height = 10}
...
```{R  plot permutation pre vs model2, fig.height = 10}
...
```{R plot permutation pre vs model3, fig.height = 10}
...
Model with variable selection using p(corr) corresponding to p-value less than `r p_pearson_of_pcorr_cutoff` which gives model with high Q2, low difference between R2 and Q2, low pQ2 post variable selection and using minimal amount of variables
...
```{R plot permutation pre vs model4, fig.height = 10}
...
# Iteration table
## Create iteration table continuously increasing p(corr) during variable selection and adding orthogonals only if it increases Q2 1% in the model (i.e. not concidering the total amount of orthogonals pre and post)

```{R iteration table}
testvariablename <- "iterationtable"
testvariable <- exist_in_rdata(testvariablename, filename_rdata, directory_output_reports, directory_and_filename_rdata)
if (!is.null(testvariable)){iterationtable <- testvariable} else {
...
plotscore(result_iterationmodel)

plotboxplot(result_iterationmodel)
