...
summary_Model_tabletokable[,c(9,10,11,12)] <- 
  format(round(summary_Model_tabletokable[,c(9,10,11,12)], digits=3),nsmall=3)
...
summary_Model_tablesignificant <- subset(summary_Model_table,summary_Model_table$`pQ2 permutated over v.s.`<=0.05 & summary_Model_table$`R2Y(cum)`>=0.5 & summary_Model_table$`Q2(cum)`>=0.4 & as.numeric(summary_Model_table$`Corr perm. coeff Q2`)>=0.1&as.numeric(summary_Model_table$`Corr perm. p value Q2`)<=0.05)
summary_Model_tablesignificanttokable <- summary_Model_tablesignificant
summary_Model_tablesignificanttokable[,c(9,10,11,12)] <- format(round(summary_Model_tablesignificanttokable[,c(9,10,11,12)],3),nsmall=3)
summary_Model_tablesignificanttokable[,c(19,20,21)] <- format(round(summary_Model_tablesignificanttokable[,c(19,20,21)],2),nsmall=2)
summary_Model_tablesignificanttokable[,c(22,23)] <- format(summary_Model_tablesignificanttokable[,c(22,23)], scientific=TRUE, digits=2)
