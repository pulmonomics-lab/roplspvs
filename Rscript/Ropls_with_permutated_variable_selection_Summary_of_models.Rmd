...
author: |
    | Ropls with permutated variable selection: Version 12
    | Marika Strom
date: "`r Sys.Date()`"
---
...
Permutations pre variable selection: `r no_permutations_pre_vs`
...
meanofR2 <- mean(summary_Model1_table$`pR2Y permutated pre v.s.`)
meanofQ2 <- mean(summary_Model1_table$`pQ2 permutated pre v.s.`)
meanofR2 <- format(meanofR2, digits = 2)
meanofQ2 <- format(meanofQ2, digits = 2)

```
Average of pR2Y respectively pQ2 of models with permutations pre v.s.:
pR2Y: `r meanofR2`
pQ2: `r meanofQ2`
...
# Model 1. Models with userset p(corr) cutoff during variable selection and userset number of orthogonal variables pre and post variable selection. 
Showing only significant models (p(Q2_perm_pre_vs) <= 0.05 and R2Y >= 0.5 and Q2 >= 0.4)

```{r, echo=FALSE}

summary_Model1_tablesignificant <- subset(summary_Model1_table,summary_Model1_table$`pQ2 permutated pre v.s.`<=0.05 & summary_Model1_table$`R2Y(cum)`>=0.5 & summary_Model1_table$`Q2(cum)`>=0.4)
...
#Model 2. Model with p(corr) cutoff corresponding to userset pvalue cutoff for pearson correlation `r p_pearson_of_pcorr_cutoff`


```{r Final models, echo=FALSE}
...
kable(summary_Model2_tabletokable, row.names = FALSE) %>%
  kable_styling(bootstrap_options = "striped", full_width = F)
meanofR2 <- mean(summary_Model2_table$`pR2Y permutated pre v.s.`)
meanofQ2 <- mean(summary_Model2_table$`pQ2 permutated pre v.s.`)
meanofR2 <- format(meanofR2, digits = 2)
meanofQ2 <- format(meanofQ2, digits = 2)
...
summary_Model2_tablesignificant <- subset(summary_Model2_table,summary_Model2_table$`pQ2 permutated pre v.s.`<=0.05 & summary_Model2_table$`R2Y(cum)`>=0.5 & summary_Model2_table$`Q2(cum)`>=0.4)
...
```{r model with highest Q2 and correspond to at least p[Pearson] 0.05, echo=FALSE}
...
kable(summary_Model3_tabletokable, row.names = FALSE) %>%
  kable_styling(bootstrap_options = "striped", full_width = F)
meanofR2 <- mean(summary_Model3_table$`pR2Y permutated pre v.s.`)
meanofQ2 <- mean(summary_Model3_table$`pQ2 permutated pre v.s.`)
meanofR2 <- format(meanofR2, digits = 2)
meanofQ2 <- format(meanofQ2, digits = 2)
...
summary_Model3_tablesignificant <- subset(summary_Model3_table,summary_Model3_table$`pQ2 permutated pre v.s.`<=0.05 & summary_Model3_table$`R2Y(cum)`>=0.5 & summary_Model3_table$`Q2(cum)`>=0.4)
...
```{r model with highest Q2 and correspond to at least p[Pearson] 0.05 using few variables, echo=FALSE}
...
kable(summary_Model4_tabletokable, row.names = FALSE) %>%
  kable_styling(bootstrap_options = "striped", full_width = F)
meanofR2 <- mean(summary_Model4_table$`pR2Y permutated pre v.s.`)
meanofQ2 <- mean(summary_Model4_table$`pQ2 permutated pre v.s.`)
meanofR2 <- format(meanofR2, digits = 2)
meanofQ2 <- format(meanofQ2, digits = 2)
...
summary_Model4_tablesignificant <- subset(summary_Model4_table,summary_Model4_table$`pQ2 permutated pre v.s.`<=0.05 & summary_Model4_table$`R2Y(cum)`>=0.5 & summary_Model4_table$`Q2(cum)`>=0.4)
...
```{r each comprison models, echo=FALSE, results='asis'}
...
row.names(list_of_table_of_each_comparison_df) <- c("Model 1","Model 2","Model 3","Model 4")
...
print(kable(list_of_table_of_each_comparison_dftokable) %>%
...
## Table of loadings for models with p(corr) corresponding to p[Pearson] `r p_pearson_of_pcorr_cutoff` 

```{r table of loadings pcorrcutoff with pvalue 005}
...
write.table(summarypcorrlist_Model2,paste("Summary", projectname, date_of_analysis,"pcorr_list_model_with_pcorr_cutoff_at_ppearson.txt",sep="_"),row.names=F,quote=F,sep="\t")
...
```
## Table of loadings for models using p(corr) that give best performing models with high Q2 and correspond to at least p[Pearson] `r p_pearson_of_pcorr_cutoff` 

```{r table of loadings pcorrcutoff with highest Q2 and with pvalue 005}
...
write.table(summarypcorrlist_Model3,paste("Summary",projectname, date_of_analysis,"pcorr_list_model_with_pcorr_cutoff_highestQ2_and_less_than_ppearson.txt",sep="_"),row.names=F,quote=F,sep="\t")
...
```
## Table of loadings for models using p(corr) that give best performing models with high Q2 and correspond to at least p[Pearson] `r p_pearson_of_pcorr_cutoff` using few variables

```{r table of loadings pcorrcutoff with highest Q2 and with pvalue 005 using few variables}
...
write.table(summarypcorrlistmodel_Model4,paste("Summary",projectname, date_of_analysis,"pcorr_list_model_with_pcorr_cutoff_highestQ2_and_less_than_ppearson_and_few_variables.txt",sep="_"),row.names=F,quote=F,sep="\t")
